...
app.eandB2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('eandB2');

// START_CUSTOM_CODE_eandB2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_eandB2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('eandB2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('eandB2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        eandB2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || eandB2Model.originalItem;

                app.mobileApp.navigate('#components/eandB2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/eandB2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = eandB2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                eandB2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                eandB2Model.set('originalItem', itemModel);
                eandB2Model.set('currentItem',
                    eandB2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = eandB2Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = eandB2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363889735", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = eandB2Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.eandB2.eandB2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.eandB2.eandB2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.eandB2.eandB2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.eandB2.eandB2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.eandB2.eandB2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1EB").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkEB").attr("checked", true);
                                $("#comments1FieldEB").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2EB").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkEB").attr("checked", true);
                                $("#comments2FieldEB").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3EB").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkEB").attr("checked", true);
                                $("#comments3FieldEB").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4EB").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkEB").attr("checked", true);
                                $("#comments4FieldEB").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5EB").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkEB").attr("checked", true);
                                $("#comments5FieldEB").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedEB").attr("checked", true);
                     }
            ...
            /// start edit form show
             $("#comments1FieldEB").hide();
            $("#comments2FieldEB").hide();
            $("#comments3FieldEB").hide();
            $("#comments4FieldEB").hide();
            $("#comments5FieldEB").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            var that = this,
                ...
                dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureEB").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('eandB2Model', eandB2Model);
            var param = parent.get('eandB2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('eandB2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = eandB2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!eandB2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = eandB2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                eandB2Model.set('dataSource', dataSource);
...
// START_CUSTOM_CODE_eandB2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.eandB2.eandB2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.eandB2.eandB2Model,
