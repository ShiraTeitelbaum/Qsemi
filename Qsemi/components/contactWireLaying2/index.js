...
app.contactWireLaying2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('contactWireLaying2');

// START_CUSTOM_CODE_contactWireLaying2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_contactWireLaying2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('contactWireLaying2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('contactWireLaying2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        contactWireLaying2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || contactWireLaying2Model.originalItem;

                app.mobileApp.navigate('#components/contactWireLaying2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/contactWireLaying2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = contactWireLaying2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                contactWireLaying2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                contactWireLaying2Model.set('originalItem', itemModel);
                contactWireLaying2Model.set('currentItem',
                    contactWireLaying2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = contactWireLaying2Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before 
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = contactWireLaying2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363889678", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = contactWireLaying2Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.contactWireLaying2.contactWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.contactWireLaying2.contactWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.contactWireLaying2.contactWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.contactWireLaying2.contactWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.contactWireLaying2.contactWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1CWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkCWL").attr("checked", true);
                                $("#comments1FieldCWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2CWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkCWL").attr("checked", true);
                                $("#comments2FieldCWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3CWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkCWL").attr("checked", true);
                                $("#comments3FieldCWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4CWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkCWL").attr("checked", true);
                                $("#comments4FieldCWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5CWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkCWL").attr("checked", true);
                                $("#comments5FieldCWL").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedCWL").attr("checked", true);
                     }
            ...
            $("#comments1FieldCWL").hide();
            $("#comments2FieldCWL").hide();
            $("#comments3FieldCWL").hide();
            $("#comments4FieldCWL").hide();
            $("#comments5FieldCWL").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            var that = this,
                ...
                dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureCWL").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('contactWireLaying2Model', contactWireLaying2Model);
            var param = parent.get('contactWireLaying2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('contactWireLaying2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = contactWireLaying2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!contactWireLaying2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = contactWireLaying2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                contactWireLaying2Model.set('dataSource', dataSource);
...
})(app.contactWireLaying2);

// START_CUSTOM_CODE_contactWireLaying2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.contactWireLaying2.contactWireLaying2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.contactWireLaying2.contactWireLaying2Model,
...
*/
// END_CUSTOM_CODE_contactWireLaying2Model