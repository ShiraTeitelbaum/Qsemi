...
app.counterwWeight2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('counterwWeight2');

// START_CUSTOM_CODE_counterwWeight2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_counterwWeight2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('counterwWeight2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('counterwWeight2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        counterwWeight2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || counterwWeight2Model.originalItem;

                app.mobileApp.navigate('#components/counterwWeight2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/counterwWeight2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = counterwWeight2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                counterwWeight2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                counterwWeight2Model.set('originalItem', itemModel);
                counterwWeight2Model.set('currentItem',
                    counterwWeight2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = counterwWeight2Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = counterwWeight2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363889696", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = counterwWeight2Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.counterwWeight2.counterwWeight2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.counterwWeight2.counterwWeight2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.counterwWeight2.counterwWeight2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.counterwWeight2.counterwWeight2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.counterwWeight2.counterwWeight2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1CW").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkCW").attr("checked", true);
                                $("#comments1FieldCW").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2CW").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkCW").attr("checked", true);
                                $("#comments2FieldCW").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3CW").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkCW").attr("checked", true);
                                $("#comments3FieldCW").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4CW").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkCW").attr("checked", true);
                                $("#comments4FieldCW").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5CW").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkCW").attr("checked", true);
                                $("#comments5FieldCW").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedCW").attr("checked", true);
                     }
            ...
            $("#comments1FieldCW").hide();
            $("#comments2FieldCW").hide();
            $("#comments3FieldCW").hide();
            $("#comments4FieldCW").hide();
            $("#comments5FieldCW").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            var that = this,
                ...
                dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureCW").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('counterwWeight2Model', counterwWeight2Model);
            var param = parent.get('counterwWeight2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('counterwWeight2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = counterwWeight2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!counterwWeight2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = counterwWeight2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                counterwWeight2Model.set('dataSource', dataSource);
...
// START_CUSTOM_CODE_counterwWeight2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.counterwWeight2.counterwWeight2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.counterwWeight2.counterwWeight2Model,
...
*/
// END_CUSTOM_CODE_counterwWeight2Model