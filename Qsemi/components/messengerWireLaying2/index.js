...
app.messengerWireLaying2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('messengerWireLaying2');

// START_CUSTOM_CODE_messengerWireLaying2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_messengerWireLaying2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('messengerWireLaying2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('messengerWireLaying2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        messengerWireLaying2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || messengerWireLaying2Model.originalItem;

                app.mobileApp.navigate('#components/messengerWireLaying2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/messengerWireLaying2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = messengerWireLaying2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                messengerWireLaying2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                messengerWireLaying2Model.set('originalItem', itemModel);
                messengerWireLaying2Model.set('currentItem',
                    messengerWireLaying2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = messengerWireLaying2Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = messengerWireLaying2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363889745", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = messengerWireLaying2Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.messengerWireLaying2.messengerWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.messengerWireLaying2.messengerWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.messengerWireLaying2.messengerWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.messengerWireLaying2.messengerWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.messengerWireLaying2.messengerWireLaying2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1MWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkMWL").attr("checked", true);
                                $("#comments1FieldMWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2MWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkMWL").attr("checked", true);
                                $("#comments2FieldMWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3MWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkMWL").attr("checked", true);
                                $("#comments3FieldMWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4MWL").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkMWL").attr("checked", true);
                                $("#comments4FieldMWL").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                ...
                                $("#checkbox5NotOkMWL").attr("checked", true);
                                $("#comments5FieldMWL").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedMWL").attr("checked", true);
                     }
            ...
            /// start edit form show
             $("#comments1FieldMWL").hide();
            $("#comments2FieldMWL").hide();
            $("#comments3FieldMWL").hide();
            $("#comments4FieldMWL").hide();
            $("#comments5FieldMWL").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            var that = this,
                ...
                dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureMWL").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('messengerWireLaying2Model', messengerWireLaying2Model);
            var param = parent.get('messengerWireLaying2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('messengerWireLaying2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = messengerWireLaying2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!messengerWireLaying2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = messengerWireLaying2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                messengerWireLaying2Model.set('dataSource', dataSource);
...
})(app.messengerWireLaying2);

// START_CUSTOM_CODE_messengerWireLaying2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.messengerWireLaying2.messengerWireLaying2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.messengerWireLaying2.messengerWireLaying2Model,
...
*/
// END_CUSTOM_CODE_messengerWireLaying2Model