...
app.generalAdjustement2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('generalAdjustement2');

// START_CUSTOM_CODE_generalAdjustement2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_generalAdjustement2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('generalAdjustement2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('generalAdjustement2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        generalAdjustement2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || generalAdjustement2Model.originalItem;

                app.mobileApp.navigate('#components/generalAdjustement2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/generalAdjustement2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = generalAdjustement2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                generalAdjustement2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                generalAdjustement2Model.set('originalItem', itemModel);
                generalAdjustement2Model.set('currentItem',
                    generalAdjustement2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = generalAdjustement2Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = generalAdjustement2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363889716", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = generalAdjustement2Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.generalAdjustement2.generalAdjustement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.generalAdjustement2.generalAdjustement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.generalAdjustement2.generalAdjustement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.generalAdjustement2.generalAdjustement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.generalAdjustement2.generalAdjustement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1GA").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkGA").attr("checked", true);
                                $("#comments1FieldGA").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2GA").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkGA").attr("checked", true);
                                $("#comments2FieldPSDTPGA").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3GA").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkGA").attr("checked", true);
                                $("#comments3FieldGA").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4GA").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkGA").attr("checked", true);
                                $("#comments4FieldGA").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5GA").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkGA").attr("checked", true);
                                $("#comments5FieldGA").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedGA").attr("checked", true);
                     }
            ...
            $("#comments1FieldGA").hide();
            $("#comments2FieldGA").hide();
            $("#comments3FieldGA").hide();
            $("#comments4FieldGA").hide();
            $("#comments5FieldGA").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
        ...
        onSaveClick: function(e) {
            var that = this,
                ...
                dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureGA").jSignature("getData"));
            ...
            itemData.set('Comments2', editFormData.comments2);
            itemData.set('Comments1', editFormData.comments1);
            itemData.set('Comments5', !!editFormData.completed);
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('generalAdjustement2Model', generalAdjustement2Model);
            var param = parent.get('generalAdjustement2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('generalAdjustement2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = generalAdjustement2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!generalAdjustement2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = generalAdjustement2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                generalAdjustement2Model.set('dataSource', dataSource);
...
})(app.generalAdjustement2);

// START_CUSTOM_CODE_generalAdjustement2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.generalAdjustement2.generalAdjustement2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.generalAdjustement2.generalAdjustement2Model,
...
*/
// END_CUSTOM_CODE_generalAdjustement2Model