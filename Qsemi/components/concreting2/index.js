...
app.concreting2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('concreting2');

// START_CUSTOM_CODE_concreting2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_concreting2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('concreting2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('concreting2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        concreting2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || concreting2Model.originalItem;

                app.mobileApp.navigate('#components/concreting2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/concreting2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = concreting2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                concreting2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                concreting2Model.set('originalItem', itemModel);
                concreting2Model.set('currentItem',
                    concreting2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = concreting2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363889669", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    var checkBox1List = app.concreting2.concreting2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     ...
                     var checkBox3List = app.concreting2.concreting2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.concreting2.concreting2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.concreting2.concreting2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1C").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkC").attr("checked", true);
                                $("#comments1FieldC").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2C").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkC").attr("checked", true);
                                $("#comments2FieldC").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3C").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkC").attr("checked", true);
                                $("#comments3FieldC").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4C").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkC").attr("checked", true);
                                $("#comments4FieldC").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5C").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkC").attr("checked", true);
                                $("#comments5FieldC").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedC").attr("checked", true);
                     }
            ...
            $("#comments1FieldC").hide();
            $("#comments2FieldC").hide();
            $("#comments3FieldC").hide();
            $("#comments4FieldC").hide();
            $("#comments5FieldC").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = concreting2Model.get('_jsdoOptions'),
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureC").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('concreting2Model', concreting2Model);
            var param = parent.get('concreting2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('concreting2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = concreting2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!concreting2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = concreting2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                concreting2Model.set('dataSource', dataSource);
...
// START_CUSTOM_CODE_concreting2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.concreting2.concreting2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.concreting2.concreting2Model,
...
*/
// END_CUSTOM_CODE_concreting2Model