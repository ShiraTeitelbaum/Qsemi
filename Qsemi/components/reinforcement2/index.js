...
app.reinforcement2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('reinforcement2');

// START_CUSTOM_CODE_reinforcement2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_reinforcement2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('reinforcement2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('reinforcement2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        reinforcement2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || reinforcement2Model.originalItem;

                app.mobileApp.navigate('#components/reinforcement2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/reinforcement2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = reinforcement2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                reinforcement2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                reinforcement2Model.set('originalItem', itemModel);
                reinforcement2Model.set('currentItem',
                    reinforcement2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = reinforcement2Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = reinforcement2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363890821", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    var checkBox1List = app.reinforcement2.reinforcement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.reinforcement2.reinforcement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.reinforcement2.reinforcement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.reinforcement2.reinforcement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.reinforcement2.reinforcement2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1R").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkR").attr("checked", true);
                                $("#comments1FieldR").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2R").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkR").attr("checked", true);
                                $("#comments2FieldR").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3R").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkR").attr("checked", true);
                                $("#comments3FieldR").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4R").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkR").attr("checked", true);
                                $("#comments4FieldR").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5R").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkR").attr("checked", true);
                                $("#comments5FieldR").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedR").attr("checked", true);
                     }
            ...
            $("#comments1FieldR").hide();
            $("#comments2FieldR").hide();
            $("#comments3FieldR").hide();
            $("#comments4FieldR").hide();
            $("#comments5FieldR").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = reinforcement2Model.get('_jsdoOptions'),
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureR").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('reinforcement2Model', reinforcement2Model);
            var param = parent.get('reinforcement2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('reinforcement2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = reinforcement2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!reinforcement2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = reinforcement2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                reinforcement2Model.set('dataSource', dataSource);
...
// START_CUSTOM_CODE_reinforcement2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.reinforcement2.reinforcement2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.reinforcement2.reinforcement2Model,
...
*/
// END_CUSTOM_CODE_reinforcement2Model