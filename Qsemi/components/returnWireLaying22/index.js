...
app.returnWireLaying22 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('returnWireLaying22');

// START_CUSTOM_CODE_returnWireLaying22
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_returnWireLaying22
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('returnWireLaying22Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('returnWireLaying22Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        /// start data sources
        /// end data sources
        returnWireLaying22Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || returnWireLaying22Model.originalItem;

                app.mobileApp.navigate('#components/returnWireLaying22/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/returnWireLaying22/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = returnWireLaying22Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                returnWireLaying22Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                returnWireLaying22Model.set('originalItem', itemModel);
                returnWireLaying22Model.set('currentItem',
                    returnWireLaying22Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = returnWireLaying22Model.fixHierarchicalData(itemData);

                app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = returnWireLaying22Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363890860", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = returnWireLaying22Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.returnWireLaying22.returnWireLaying22Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.returnWireLaying22.returnWireLaying22Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.returnWireLaying22.returnWireLaying22Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.returnWireLaying22.returnWireLaying22Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.returnWireLaying22.returnWireLaying22Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1RWL2").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkRWL2").attr("checked", true);
                                $("#comments1FieldRWL2").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2RWL2").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkRWL2").attr("checked", true);
                                $("#comments2FieldRWL2").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3RWL2").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkRWL2").attr("checked", true);
                                $("#comments3FieldRWL2").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4RWL2").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkRWL2").attr("checked", true);
                                $("#comments4FieldRWL2").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5RWL2").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkRWL2").attr("checked", true);
                                $("#comments5FieldRWL2").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedRWL2").attr("checked", true);
                     }
            ...
            $("#comments1FieldRWL2").hide();
            $("#comments2FieldRWL2").hide();
            $("#comments3FieldRWL2").hide();
            $("#comments4FieldRWL2").hide();
            $("#comments5FieldRWL2").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
            ...
            var that = this,
                ...
                dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('signature', $("#signatureRWL2").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('returnWireLaying22Model', returnWireLaying22Model);
            var param = parent.get('returnWireLaying22Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('returnWireLaying22Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = returnWireLaying22Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!returnWireLaying22Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = returnWireLaying22Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                returnWireLaying22Model.set('dataSource', dataSource);
...
})(app.returnWireLaying22);

// START_CUSTOM_CODE_returnWireLaying22Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.returnWireLaying22.returnWireLaying22Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.returnWireLaying22.returnWireLaying22Model,
...
*/
// END_CUSTOM_CODE_returnWireLaying22Model