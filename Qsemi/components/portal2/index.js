...
app.portal2 = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});
app.localization.registerView('portal2');

// START_CUSTOM_CODE_portal2
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// END_CUSTOM_CODE_portal2
(function(parent) {
    var dataProvider = app.data.qcsemidataProvider,
        ...
        fetchFilteredData = function(paramFilter, searchFilter) {
            var model = parent.get('portal2Model'),
                dataSource;

            if (model) {
                dataSource = model.get('dataSource');
            } else {
                parent.set('portal2Model_delayedFetch', paramFilter || null);
                return;
            }
        ...
        portal2Model = kendo.observable({
            ...
            itemClick: function(e) {
                var dataItem = e.dataItem || portal2Model.originalItem;

                app.mobileApp.navigate('#components/portal2/details.html?uid=' + dataItem.uid);

            },
            editClick: function() {
                var uid = this.originalItem.uid;
                app.mobileApp.navigate('#components/portal2/edit.html?uid=' + uid);
            },
            detailsShow: function(e) {
                var uid = e.view.params.uid,
                    dataSource = portal2Model.get('dataSource'),
                    itemModel = dataSource.getByUid(uid);

                portal2Model.setCurrentItemByUid(uid);
            ...
            setCurrentItemByUid: function(uid) {
                ...
                portal2Model.set('originalItem', itemModel);
                portal2Model.set('currentItem',
                    portal2Model.fixHierarchicalData(itemModel));
    ...
    parent.set('editItemViewModel', kendo.observable({
        ...
        onShow: function(e) {
            ...
            var that = this,
                ...
                //fixedData = portal2Model.fixHierarchicalData(itemData);

                 app.mobileApp.showLoading();
            /// start edit form before itemData
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions2 = portal2Model.get('_jsdoOptions'),
                ...
                dataSource2.filter({ field: "R363890812", operator: "==", value: element_id });
                ...
                dataSource2.fetch(function() {
                    ...
                    fixedData = portal2Model.fixHierarchicalData(itemData);
                     ...
                     var checkBox1List = app.portal2.portal2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox1().response.picklistData;
                     var checkBox2List = app.portal2.portal2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox2().response.picklistData;
                     var checkBox3List = app.portal2.portal2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox3().response.picklistData;
                     var checkBox4List = app.portal2.portal2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox4().response.picklistData;
                     var checkBox5List = app.portal2.portal2Model._dataSourceOptions.transport.jsdo.getPicklist_Checkbox5().response.picklistData;
                     ...
                     if(itemData.Checkbox1 != "null") {
                        for(var i=0; i<checkBox1List.length; i++) {
                            if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "OK") {
                                $("#checkbox1Po").attr("checked", true);
                            }
                            else if(itemData.Checkbox1 == checkBox1List[i].id && checkBox1List[i].name == "Not OK") {
                                $("#checkbox1NotOkPo").attr("checked", true);
                                $("#comments1FieldPo").show();
                            }
                        }
                     }
                     if(itemData.Checkbox2 != "null") {
                        for(var i=0; i<checkBox2List.length; i++) {
                            if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "OK") {
                                $("#checkbox2Po").attr("checked", true);
                            }
                            else if(itemData.Checkbox2 == checkBox2List[i].id && checkBox2List[i].name == "Not OK") {
                                $("#checkbox2NotOkPo").attr("checked", true);
                                $("#comments2FieldPo").show();
                            }
                        }
                     }
                     if(itemData.Checkbox3 != "null") {
                        for(var i=0; i<checkBox3List.length; i++) {
                            if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "OK") {
                                $("#checkbox3Po").attr("checked", true);
                            }
                            else if(itemData.Checkbox3 == checkBox3List[i].id && checkBox3List[i].name == "Not OK") {
                                $("#checkbox3NotOkPo").attr("checked", true);
                                $("#comments3FieldPo").show();
                            }
                        }
                     }
                     if(itemData.Checkbox4 != "null") {
                        for(var i=0; i<checkBox4List.length; i++) {
                            if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "OK") {
                                $("#checkbox4Po").attr("checked", true);
                            }
                            else if(itemData.Checkbox4 == checkBox4List[i].id && checkBox4List[i].name == "Not OK") {
                                $("#checkbox4NotOkPo").attr("checked", true);
                                $("#comments4FieldPo").show();
                            }
                        }
                     }
                     if(itemData.Checkbox5 != "null") {
                        for(var i=0; i<checkBox5List.length; i++) {
                            if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "OK") {
                                $("#checkbox5Po").attr("checked", true);
                            }
                            else if(itemData.Checkbox5 == checkBox5List[i].id && checkBox5List[i].name == "Not OK") {
                                $("#checkbox5NotOkPo").attr("checked", true);
                                $("#comments5FieldPo").show();
                            }
                        }
                     }

                     if(itemData.Completed == true) {
                         $("#completedPo").attr("checked", true);
                     }
            ...
            $("#comments1FieldPo").hide();
            $("#comments2FieldPo").hide();
            $("#comments3FieldPo").hide();
            $("#comments4FieldPo").hide();
            $("#comments5FieldPo").hide();
            /// end edit form show
        },
        linkBind: function(linkString) {
            var linkChunks = linkString.split(':');
                ...
                var jsdoOptions2 = portal2Model.get('_jsdoOptions'),
                    ...
                    dataSource2.fetch(function() {
                        ...
                        itemData.set('Comments1', editFormData.comments1);
                         itemData.set('signature', $("#signaturePo").jSignature("getData"));
    ...
    if (typeof dataProvider.sbProviderReady === 'function') {
        dataProvider.sbProviderReady(function dl_sbProviderReady() {
            parent.set('portal2Model', portal2Model);
            var param = parent.get('portal2Model_delayedFetch');
            if (typeof param !== 'undefined') {
                parent.set('portal2Model_delayedFetch', undefined);
    ...
    parent.set('onShow', function(e) {
        var param = e.view.params.filter ? JSON.parse(e.view.params.filter) : null,
            ...
            dataSourceOptions = portal2Model.get('_dataSourceOptions'),
            dataSource;
        ...
        if (!portal2Model.get('dataSource')) {
            dataProvider.loadCatalogs().then(function _catalogsLoaded() {
                var jsdoOptions = portal2Model.get('_jsdoOptions'),
                    jsdo = new progress.data.JSDO(jsdoOptions);

                dataSourceOptions.transport.jsdo = jsdo;
                dataSource = new kendo.data.DataSource(dataSourceOptions);
                portal2Model.set('dataSource', dataSource);
...
// START_CUSTOM_CODE_portal2Model
// Add custom code here. For more information about custom code, see http://docs.telerik.com/platform/screenbuilder/troubleshooting/how-to-keep-custom-code-changes

// you can handle the beforeFill / afterFill events here. For example:
/*
app.portal2.portal2Model.get('_jsdoOptions').events = {
    'beforeFill' : [ {
        scope : app.portal2.portal2Model,
